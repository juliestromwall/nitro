...
| CompanySales | src/components/company/CompanySales.jsx | Per-company sales: season tabs ("+ New Sales Tracker"), hover pencil icon to edit tab (name/year/dates), edit modal with Archive/Unarchive button, archived dropdown with edit icons, search, filters, inline edit, notes modal (StickyNote icon), scrollable table |
| CompanyCommission | src/components/company/CompanyCommission.jsx | Per-company commission: order-driven (Closed - Won orders), Order #/Invoice #/Total columns, clickable summary card filters (Earned/Paid/Outstanding), row highlighting (green=Paid, yellow=Partial), search bar, hover pencil icon to edit tab, edit modal with Archive/Unarchive button, archived dropdown with edit icons, inline pay status editing (dropdown + amount paid + paid date) |
