...
| CompanySales | src/components/company/CompanySales.jsx | Per-company sales: season tabs, sticky first-column edit/delete actions, 4 clickable summary cards that filter by order type, 2-step Add/Edit Sale wizard (Step 1: Sale Type, Tracker, Account; Step 2: Order Type, Items, Order #, Total, Stage, Close Date, Notes), Sale Type column in table (Prebook/At Once), document upload on Order #, invoice # hyperlinks with signed URLs, prominent currency-formatted Total input with live comma formatting ($X,XXX.XX) and large $ sign, required Order Type and Stage fields, Stage next to Close Date, company logo in modal footer, celebration popup after adding a sale (animated confetti, bouncing company logo, random hype messages, commission amount in green, "LET'S GO!" dismiss), "+Note" text button vs amber StickyNote icon, search, filters, notes modal, CSV import with tracker confirmation dialog |
