...
| CompanyCommission | src/components/company/CompanyCommission.jsx | Per-company commission: order-driven (excludes Cancelled and Short Shipped), **account-grouped table** (orders grouped by account with header rows showing account name, order count, combined total, combined commission due, invoices with doc links and pending amount), sticky first-column edit actions with status-aware backgrounds, clickable summary card filters (Earned/Paid/Outstanding), row highlighting (green=Paid, yellow=Partial), search bar, inline pay status editing persisted to Supabase |
