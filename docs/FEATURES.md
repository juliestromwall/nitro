...
| CompanyCommission | src/components/company/CompanyCommission.jsx | Per-company commission: order-driven (excludes Cancelled and Short Shipped), sticky first-column edit actions with status-aware backgrounds, clickable summary card filters (Earned/Paid/Outstanding), row highlighting (green=Paid, yellow=Partial), stacked order/invoice numbers with document hyperlinks and pending indicator, search bar, inline pay status editing persisted to Supabase |
