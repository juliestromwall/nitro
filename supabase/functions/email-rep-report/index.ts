...
// (via domain-wide delegation). Everyone else falls back to the shared
// accounting@ mailbox below.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // the shared accounting@ mailbox below, so a send is never lost.
    ...
    if (gmailDwdConfigured() && senderEmail.endsWith(`@${GMAIL_SEND_DOMAIN}`)) {
      try {
        ...
        console.error('Gmail send-as-user failed, falling back to accounting@:', gmailErr)
      }
    }
